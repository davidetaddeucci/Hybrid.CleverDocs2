# PowerShell script to test heavy bulk upload (20 x 2MB files) - Simplified version
param(
...
# Step 1: Login via WebUI
Write-Host "`n🔐 Step 1: Authenticating via WebUI..." -ForegroundColor Yellow
...
try {
    ...
    # Extract anti-forgery token using simpler regex
    $tokenMatch = [regex]::Match($loginPage, '__RequestVerificationToken.*?value="([^"]+)"')
    ...
    # Perform login
    Write-Host "Performing login..." -ForegroundColor Gray
    $loginResponse = Invoke-WebRequest -Uri $loginUrl -Method POST -Body $loginFormData -WebSession $session -ContentType "application/x-www-form-urlencoded"
...
try {
    ...
    $uploadTokenMatch = [regex]::Match($uploadPage, '__RequestVerificationToken.*?value="([^"]+)"')
    if ($uploadTokenMatch.Success) {
        $uploadAntiForgeryToken = $uploadTokenMatch.Groups[1].Value
...
# Step 3: Prepare and execute bulk upload using JavaScript approach
...
try {
    ...
    foreach ($file in $testFiles) {
        $fileSizeMB = [math]::Round($file.Length / (1024*1024), 2)
    ...
    $requestSizeMB = [math]::Round($body.Length / (1024*1024), 2)
    ...
    if ($uploadResponse.StatusCode -eq 200) {
        ...
        if ($result.success) {
            ...
            if ($result.collectionId) {
                ...
                # Open browser to collection page for manual verification
                Write-Host "`n🌐 Opening browser to Collection Detail page..." -ForegroundColor Yellow
                Start-Process "$WebUIUrl/Collections/$($result.collectionId)"
            }
        } else {
