...
namespace Hybrid.CleverDocs2.WebServices.Controllers
{
    ...
    public class LocalLLMController : ControllerBase
    {
        ...
        public LocalLLMController(ILocalLLMClient client) => _client = client;

        [HttpPost]
        public async Task<IActionResult> Create(LocalLLMRequest request) => Ok(await _client.CreateAsync(request));
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(string id, LocalLLMRequest request) => Ok(await _client.UpdateAsync(id, request));
